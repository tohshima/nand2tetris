...
class CodeWriter:
    ...
    def setFileName(self, fileName):
        self.__writeToList = False
        self.__filename_or_testLines = fileName
        self.close()
    ...
    def __makeCallCode(self, functionName, nArgs, code:list):
        ...
        code.append(f'// Store the caller pointer LCL/ARG/THIS/THAT) to the stack frame')
        code.append(f'@LCL    // {self.__pc+0: >5}:A=&LCL:1')
        code.append(f'A=M     // {self.__pc+1: >5}:A=LCL')
        code.append(f'D=M     // {self.__pc+2: >5}:D=*LCL')
        code.append(f'@SP     // {self.__pc+3: >5}:A=&SP:0')
        code.append(f'A=M     // {self.__pc+4: >5}:A=SP')
        code.append(f'M=D     // {self.__pc+5: >5}:*SP=*LCL')
        code.append(f'D=A     // {self.__pc+6: >5}:D=SP')
        code.append(f'@LCL    // {self.__pc+7: >5}:A=&LCL:1')
        code.append(f'M=D     // {self.__pc+8: >5}:LCL=SP:newLCL')
        self.__pc += 9
    ...
    def __makeReturnCode(self, code:list):
        ...
        self.__makePopCode("argument", 0, code, "// Pop the return value of the function to ARG")
        code.append(f'@ARG    // {self.__pc+0: >5}:A=&ARG:2')
        code.append(f'D=M+1   // {self.__pc+1: >5}:D=ARG+1')
        code.append(f'@SP     // {self.__pc+2: >5}:A=&SP:0')
        code.append(f'M=D     // {self.__pc+3: >5}:SP=ARG+1')
        self.__pc += 4  
        ...
        code.append(f'@R13    // {self.__pc+11: >5}:A=&R13')
        code.append(f'A=M     // {self.__pc+12: >5}:A=R13:return address')
        code.append(f'0;JMP   // {self.__pc+13: >5}:jump to the return address')
